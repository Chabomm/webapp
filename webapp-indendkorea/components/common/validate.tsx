export default function validate(values: any) {
    ...
    Object.keys(values).map((v, i) => {
        ...
        if ((el as HTMLElement)?.getAttribute('is_email') === 'true' && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values[v])) {
            errors[v] = '입력된 이메일이 유효하지 않습니다.';
            focusing.push(v);
        }
        if ((el as HTMLElement)?.getAttribute('is_bizno') === 'true' && !/^(\d{3,3})+[-]+(\d{2,2})+[-]+(\d{5,5})/i.test(values[v])) {
            errors[v] = '입력된 사업자등록번호가 유효하지 않습니다.';
            focusing.push(v);
        }
        if ((el as HTMLElement)?.getAttribute('is_mobile') === 'true' && !/^(01[016789]{1}|02|0[3-9]{1}[0-9]{1})-?[0-9]{3,4}-?[0-9]{4}$/i.test(values[v])) {
