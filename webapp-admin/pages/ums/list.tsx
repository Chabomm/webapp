...
import UmsEdit from '@/components/ums/UmsEdit';

const UmsList: NextPage = (props: any) => {
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/admin/ums/template/list`, p);
            setParams((param: any) => {
    ...
    const getFilterContidion = async () => {
        try {
            const { data } = await api.post(`/be/admin/ums/template/filter`);
            setFilter(data);
            const copy = { ...s.values };
    ...
    const refUmsEdit = useRef<any>();
    function openUmsEdit(item: any) {
        refUmsEdit.current.init(item, filter);
    }

    return (
        <Layout user={props.user} title="indendkorea admin console" nav_id={16} crumbs={['UMS관리', 'UMS 템플릿']}>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/admin/ums/template/list`, request);
        response = data;
